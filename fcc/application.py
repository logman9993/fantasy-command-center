...
from fcc.cache import cached_json, stale_cached_json, timed_cache, cached_csv as shared_cached_csv
from fcc.http import HTTP, http_json, http_text
from fcc.security import extension_identity_ok

from fcc.scoring import ELIGIBLE, optimize, score_stats, scoring_rules, slots_from, waiver_moves
...
limiter = Limiter(
    ...
    storage_uri=os.getenv("REDIS_URL") or os.getenv("KEY_VALUE_URL") or "memory://",
)
...
if (YAHOO_CLIENT_ID or YAHOO_CLIENT_SECRET or YAHOO_TOKEN_SECRET) and Fernet is None:
    raise RuntimeError("Yahoo OAuth is configured but cryptography/Fernet is unavailable.")
SEASON = int(
...
def cached_csv(name, ttl, urls, must_have):
    return shared_cached_csv(name, ttl, urls, must_have, http_text)
...
@timed_cache(maxsize=12, ttl=21600)
...
def history_for(name, pos, scoring, yearly):
    ...
    for year, rows in yearly.items():
        ...
        if not r:
            continue
        from fcc.v10_api import has_points
...
def _fantasypros_rank_position(pos, scoring, limit):
    if not FANTASYPROS_KEY:
        return pos, [], None
    try:
        data = cached_json(
        ...
        return pos, normalize_fp_players(data)[:limit], None
    except Exception as exc:
        return pos, [], exc


@timed_cache(maxsize=12, ttl=900)
def rankings(scoring, limit=TOP_N):
    result = {}
    model = None
    live_by_pos = {pos: [] for pos in ALL_POSITIONS}

    if FANTASYPROS_KEY:
        with ThreadPoolExecutor(max_workers=6) as executor:
            futures = [executor.submit(_fantasypros_rank_position, pos, scoring, limit) for pos in ALL_POSITIONS]
            for future in as_completed(futures):
                pos, rows, error = future.result()
                live_by_pos[pos] = rows
                if rows:
                    source_ok(f"rank_{pos}", f"FantasyPros ECR ({len(rows)})")
                elif error:
                    source_fail(f"rank_{pos}", error)

    for pos in ALL_POSITIONS:
        live = list(live_by_pos.get(pos, []))
        if len(live) < limit:
            if model is None:
                model = build_model_rankings(scoring, limit)
            seen = {norm_name(x["name"]) for x in live}
            for player in model.get(pos, []):
                if norm_name(player["name"]) not in seen:
                    live.append(player)
                    seen.add(norm_name(player["name"]))
                if len(live) >= limit:
                    break
        if len(live) < limit:
            seen = {norm_name(x["name"]) for x in live}
            for player in fallback().get("rankings", {}).get(pos, []):
                if norm_name(player.get("name")) not in seen:
                    row = dict(player)
                    row.setdefault("source", "bundled fallback")
                    live.append(row)
                    seen.add(norm_name(row.get("name")))
                if len(live) >= limit:
                    break
        ...
        if len(result[pos]) >= min(limit, 32 if pos == "DST" else limit) and f"rank_{pos}" not in SOURCE_STATE:
...
def cache_headers(response):
    path = request.path
    if path.startswith(("/api/yahoo", "/auth/yahoo", "/api/espn", "/api/sleeper/team-analysis", "/api/manual-team-analysis")):
        response.headers["Cache-Control"] = "private, no-store"
    elif path.startswith(("/api/dashboard", "/api/rankings/", "/api/news")) and response.status_code == 200:
...
def dashboard():
    ...
    try:
        data = fallback()
        ranks = rankings(scoring, TOP_N)
        from fcc.v10_api import attach_photos
...
def position_rankings_api(position):
    ...
    if scoring not in SCORING:
        scoring = "PPR"
    try:
        limit = min(100, max(25, int(request.args.get("limit") or 100)))
    except (TypeError, ValueError):
        limit = 100
    rows = rankings(scoring, limit).get(pos, [])
    from fcc.v10_api import attach_photos
    rows = attach_photos({pos: rows}, sleeper_players(), norm_name, player_name).get(pos, [])
    return jsonify({"position": pos, "scoring": scoring, "items": rows, "updated": int(time.time())})
...
def espn_analyze_api():
    if not extension_identity_ok():
        return jsonify({"error":"extension_identity_required","message":"Request must come from the packaged Command Center ESPN extension."}), 403
...
from fcc.v9_api import register_v9

register_v9(app, globals())
from fcc.v10_api import register_v10
